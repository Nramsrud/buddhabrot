/*  Buddhabrot
    ...
    Initailly optimized and cleaned up version by Michael Pohoreski
    Based on the original version by Evercat
    Hacky patch to run multithreaded on Apple M4 Pro chip by Nicolas Ramsrud
...
int Buddhabrot()
{
    ...
    const size_t nCol = gnWidth * gnScale;   // Scaled width (columns)
    const size_t nRow = gnHeight * gnScale;    // Scaled height (rows)
    std::atomic<size_t> columnsProcessed(0);   // Atomic counter for processed columns
    ...
    std::vector<uint16_t*> localBuffers(numThreads, nullptr);
    std::vector<size_t> localCelCount(numThreads, 0);

    // Partition the nCol columns among threads.
    size_t colsPerThread = nCol / numThreads;
    size_t extraCols   = nCol % numThreads;

    // Start a progress reporting thread.
    std::thread progressThread([&]() {
        while (columnsProcessed.load() < nCol) {
            double percent = 100.0 * columnsProcessed.load() / nCol;
            printf("%6.2f%% complete\n", percent);
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
        // Ensure we print 100% once complete.
    ...
    {
        // Calculate each thread's column range.
        size_t startCol = t * colsPerThread + (t < extraCols ? t : extraCols);
        size_t endCol   = startCol + colsPerThread + (t < extraCols ? 1 : 0);
        unsigned int threadIndex = t;

        // Allocate a thread-local buffer for the greyscale image.
        localBuffers[threadIndex] = new uint16_t[gnImageArea];
        memset(localBuffers[threadIndex], 0, gnImageArea * sizeof(uint16_t));

        // Launch the worker thread.
        workerThreads.push_back(std::thread([=, &localCelCount, &localBuffers, &columnsProcessed]() {
            size_t localCount = 0;
            for (size_t iCol = startCol; iCol < endCol; iCol++)
            {
                double x = gnWorldMinX + iCol * dx;
                for (size_t iRow = 0; iRow < nRow; iRow++)
                {
                    double y = gnWorldMinY + iRow * dy;
                    double r = 0.0, im = 0.0, s, j;
                    for (int depth = 0; depth < gnMaxDepth; depth++)
                    {
                        ...
                        if ((r * r + im * im) > 4.0)
                        {
                            // Use the existing plot() function to update the thread-local buffer.
                            plot(x, y, nWorld2ImageX, nWorld2ImageY, localBuffers[threadIndex], gnWidth, gnHeight, gnMaxDepth);
                            break;
                        }
                    }
                    localCount++;
                }
                // Update the global progress counter after finishing a column.
                columnsProcessed.fetch_add(1, std::memory_order_relaxed);
            }
            localCelCount[threadIndex] = localCount;
        }));
    }
    ...
    // Combine the results from all threads into the global greyscale buffer.
    for (size_t idx = 0; idx < gnImageArea; idx++)
    {
        uint32_t sum = 0;
        for (unsigned int t = 0; t < numThreads; t++)
        {
            sum += localBuffers[t][idx];
        }
        gpGreyscaleTexels[idx] = (uint16_t)sum;
    }

    // Free the thread-local buffers.
    for (unsigned int t = 0; t < numThreads; t++)
    {
        delete[] localBuffers[t];
    }

    // Optionally, sum the total cells processed across all threads.
    size_t totalCells = 0;
    for (unsigned int t = 0; t < numThreads; t++)
    {
        totalCells += localCelCount[t];
    }
